...
for i in range(len(mu_f)):
    sol = sols[i]
    kappa = sol.y[0:n, :]            # shape (n, timepoints)
    ...
    # compute mean phase difference (phi_j) between adjacent segments FOR THIS viscosity
    phi_j_list = []
    for j in range(n-1):
        # unwrap and take (segment j+1) - (segment j) so positive means wave moves in +j direction
    ...
    # convert to numpy array and compute lambda/L for THIS viscosity
    phi_arr = np.array(phi_j_list)
    lam_over_L = ((n-1)/n) / np.sum(1.0 - phi_arr)   # your formula adapted per-viscosity
